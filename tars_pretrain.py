...
train_df.labels[train_df.labels=="enviroment/health"] = "environment and health issues"
test_df.labels[test_df.labels=="enviroment/health"] = "environment and health issues"
dev_df.labels[dev_df.labels=="enviroment/health"] = "environment and health issues"
...
#unique_labels = ['alternatives', 'reliability', 'weapons']
...
for label in unique_labels:
    ...
    for n_shot in n_shots:
        for trial in range(1,n_trials+1):
            ...
            idx_negative = train_df.labels!=label
            sample_ft = train_df[idx].sample(n=min(n_shot, sum(idx)), random_state=seed)
            sample_ft = pd.concat([sample_ft, train_df[idx_negative].sample(n=min(n_shot*10, 200), random_state=seed)])
...
for label in unique_labels:
    ...
    tars = TARSClassifier(num_negative_labels_to_sample=4, embeddings='roberta-large') # roberta-large
